...
from abridge.processor import abridge_clip
...
def mock_threadpool(mocker: MockFixture) -> None:
    ...
    mock_instance._threads = create_autospec(set)
...
def test_threadpool_executes_clips(
    ...
    mock_threadpool().submit.assert_called_with(
        abridge_clip, "somefile.mp4", "processed", 20, 5
    )
...
class TestKeyboardInterrupt:
    def test_keyboard_interrupt_shutsdown_pool(
        ...
        mock_splice = mocker.patch(
            "abridge.processor.abridge_clip", side_effect=KeyboardInterrupt()
        )
